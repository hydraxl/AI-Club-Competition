...
# Default value if function times out
...
def run_player_with_timeout(func, input, timeout=1):
    # results of player function get pushed to result variable instead of returned
    x = multiprocessing.Value('i', DEFAULT_TIMEOUT_VALUE)
    
    # Start player function as a process
    f = Player_Func(func)
    p = multiprocessing.Process(target=f, args=(input, x))
    p.start()
